...
-- UI ELEMENTS (Notifications, Watermark, Keybinds)
do
    ...
    -- Анимация для .kxn!
    local KxnAnimating = false
    function Library:AnimateKxn(Text)
        if KxnAnimating then return end
        KxnAnimating = true
        
        task.spawn(function()
            local Label = WatermarkLabel
            local OriginalText = Text
            
            while Label and Label.Parent and string.find(Label.Text, "%.kxn!") do
                -- Появление
                for i = 0, 1, 0.05 do
    ...
    local WatermarkAnimating = false
    
    function Library:AnimateWatermark(Show, Text)
        if WatermarkAnimating then return end
        WatermarkAnimating = true
        
        if Show then
            WatermarkOuter.Visible = true
            WatermarkOuter.Size = UDim2.new(0, 213, 0, 20)
            local Tween = TweenService:Create(WatermarkOuter, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Size = UDim2.new(0, 213, 0, 20)
            })
            Tween:Play()
            ...
            local Tween = TweenService:Create(WatermarkOuter, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
                Size = UDim2.new(0, 0, 0, 20)
            })
            Tween:Play()
            Tween.Completed:Wait()
            WatermarkOuter.Visible = false
            WatermarkVisible = false
        end
        
        WatermarkAnimating = false
    end
    ...
    function Library:SetWatermarkVisibility(Bool)
        Library:AnimateWatermark(Bool, Library.WatermarkText.Text)
    end;
